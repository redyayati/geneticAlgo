# Trying shalini's method for pool selection
...
class Population():
    ...
    def evaluate(self):
        ...
        for rocket in self.rockets : 
            rocket.fitness /= sum
            rocket.fitness = rocket.fitness**2
        for rocket in self.rockets : 
            cum += rocket.fitness
            rocket.cummulative = cum
    def pickOne(self) : 
        r = random.uniform(0 , self.rockets[self.popsize-1].cummulative)
        for rocket in self.rockets : 
            if r > rocket.cummulative : pass 
            else : return rocket
    def selection(self) : 
        newRockets = []
        for rocket in self.rockets : 
            parentA = self.pickOne().dna
            parentB = self.pickOne().dna
