...
class Population() : 
    ...
    def naturalSelection(self): 
        ...
        maxfitness = 0 
        for pop in self.population : 
            if pop.fit > maxfitness : 
                maxfitness = pop.fit
                self.fittestDNA = pop
        for pop in self.population : 
            fitness = pop.fit/maxfitness
            n = int(fitness * 100)
            for i in range(n) : self.matingPool.append(pop)
    def generate(self) : 
        self.xVal = []
        for i in range(len(self.population)) : 
            poollength = len(self.matingPool) - 1 
            parentA = self.matingPool[random.randint(0,poollength)]
            parentB = self.matingPool[random.randint(0,poollength)]
            child  = parentA.crossover(parentB)
            child.mutate(self.mutationRate)
            self.population[i] = child
            self.xVal.append(random.randint(450-20, 450+20))
        self.generation +=1
    def evaluate(self):
    ...
    def displayStats(self,xVal, yVal) : 
        ...
        if searching == False : 
            textMaxfit = base_fontFit.render(self.fittestDNA.getPhrase(),True,(100,100,100))
            textyay = base_font.render("there !",True,(0,0,200))
