...
class Population():
    ...
    def evaluate(self):
        ...
        # for rocket in self.rockets : rocket.fitness /= maxFit # No need to normalize as we are not creating mating pool
        self.maxfitness = maxFit
    def selection(self) : 
        newRockets = []
        for rocket in self.rockets : 
            parentA = self.acceptReject().dna
            parentB = self.acceptReject().dna
    ...
    def acceptReject(self) : 
        besafe = 0
        while True : 
            partner = random.choice(self.rockets)
            self.list.append(partner.fitness)
            r = random.uniform(0,self.maxfitness)
            if r < partner.fitness : return partner 
            besafe += 1 
            if besafe > 10000 : 
                print(self.list)
                print("ERROR : " , r , " is not less than ",partner.fitness)
                return None
...
while running : 
    ...
    pg.draw.rect(screen, ("black"), (0,0,110 ,63))
    ...
    pg.draw.circle(screen,objCol, (target.x,target.y),10)
